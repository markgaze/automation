...
Node.js is installed by default. If `node-version-file` is set, that file is used to determine the version (for example `.nvmrc` or `.node-version`). If `node-version-file` is empty, Node.js `current` is used.
